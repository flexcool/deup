...
import 'package:deup/pages/detail/layouts/cover_layout/cover_layout_item.dart';

class CoverLayout extends StatefulWidget {
  final String id;
  final bool history;
  ...
  @override
  _CoverLayoutState createState() => _CoverLayoutState();
}
...
class _CoverLayoutState extends State<CoverLayout> {
  ...
  Future<void> getFileListData(int offset) async {
    try {
      ...
      _objectList = _objectList
          .where(
              (element) => element.cover != null && element.cover!.isNotEmpty)
          .toList();
        ...
        itemBuilder: (context, item, index) {
          return FrameSeparateWidget(
            ...
            child: GestureDetector(
              ...
              child: CoverLayoutItem(object: item),
            ),
          );
        },
      ),
  ...
  Widget _buildSliverList() {
    return PagedSliverList<int, ObjectModel>.separated(
      pagingController: pagingController,
      separatorBuilder: (context, index) => Container(
        padding: EdgeInsets.symmetric(vertical: CommonUtils.isPad ? 0 : 20.h),
      ...
      slivers: <Widget>[
        ...
        SliverPadding(
          ...
          sliver: SizeCacheWidget(
            child: CommonUtils.isPad ? _buildSliverGrid() : _buildSliverList(),
          ),
        ),
      ],
    );
